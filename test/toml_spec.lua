...
describe("parse_crate_features", function()
    ...
    it("applies start_col on the first line only", function()
        local text = '"derive", "alloc"'
        local features = toml.parse_crate_features(text, 4, 10)

        assert.equals(4, features[1].line)
        assert.equals(11, features[1].col.s)
        assert.equals(17, features[1].col.e)
        assert.equals(4, features[2].line)
        assert.equals(21, features[2].col.s)
    end)
end)
